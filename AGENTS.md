...
ts/
  ...
  src/aggregate.ts     # an aggregate option value's text, as protoc records it,
                       #   and the lexer matcher that reads text format's words
  src/strings.ts       # a string written as adjacent literals, as protoc reads it
...
go/
  ...
  strings.go           # port of ts/src/strings.ts
...
rs/
  ...
  src/strings.rs       # port of ts/src/strings.ts
...
One place needs more than the grammar. Text format, which an aggregate
option value is written in, has no keywords: inside `{ message: optional }`
the field is named `message` and its value is the enum value `optional`.
The plugin therefore installs a lexer matcher, `protoAggregateWord`, at
order 9e5, ahead of the grammar's keywords at 1e6. Inside an aggregate
...
- Parsing is not sanitising. proto returns names, option values and
  ...
  below), and a string written as adjacent literals decoded as protoc
  decodes it, so such a value can hold any character, NUL and the other
  control characters included. Quoting them for SQL, HTML, a shell — or
  validating identifiers before code generation — remains the caller's
  job.
...
- An aggregate option value (`option (f) = { a: 1 };`) is a string: the
  ...
  pins it, each row checked against protoc 36.2's own parser. The text
  is cut the way protoc's tokenizer cuts it, not the way the tabnas
  lexer does, so where the two end a comment or a string differently (a
  `//` comment at a bare `\r`, a backtick string, a `/*` after `#`) it
  can leave out tokens the CST holds. On each such input measured,
  protoc either refuses it or records the same text.
- Between those braces the grammar takes what text format takes: a
  string value written as adjacent literals (`a: "x" "y"`: two or more
  are the entry's `strLit` children, and a single literal is a
  `constant`, as every other value is); a repeated field's values as a
  list (`a: [1, 2]`, a `listValue` node); a message in angle brackets
  (`a < b: 1 >`, an `angleValue` node); a field named by an extension or
  an Any type URL (`[x.y]: 1`, `[type.googleapis.com/x.Y] { a: 1 }`);
  and a field name or enum value spelt like a keyword (`message:
...
- A string may be written as adjacent literals wherever protoc reads
  ...
  (`absl::CEscape`). `ts/src/strings.ts` states the rule and
  `test/spec/adjacent-strings.tsv` pins it, each row checked against
  protoc 36.2's own parser. A string written as ONE literal is kept as
  written; see the declared deviations.
...
The first two are deliberate and bridged (not waived) by the conformance
...
3. **A string written as one literal keeps its escapes as written.**
   `option (f) = "\x41";` records `\x41`, where protoc decodes the
